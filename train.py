...
def getTokenizer(bert_mode, enc_type):
	if enc_type == 'actual_bert':
		return AutoTokenizer.from_pretrained("bert-base-uncased", output_attentions=True, output_hidden_states=False)
	elif enc_type == 'distilbert':
		return AutoTokenizer.from_pretrained("distilbert-base-uncased", output_attentions=True, output_hidden_states=False)
	elif enc_type == 'electra':
		return AutoTokenizer.from_pretrained("google/electra-base-discriminator", output_attentions=True, output_hidden_states=False)
	elif enc_type=='mbert':
		return AutoTokenizer.from_pretrained("bert-base-multilingual-uncased", output_attentions=True, output_hidden_states=False)
	elif enc_type=='xlmr':
		return AutoTokenizer.from_pretrained("xlm-roberta-base", output_attentions=True, output_hidden_states=False)		
	elif enc_type=='mdistilbert':
		return AutoTokenizer.from_pretrained("distilbert-base-multilingual-cased", output_attentions=True, output_hidden_states=False)		
	if bert_mode == 'gen':
		tokenizer = RobertaTokenizer.from_pretrained('roberta-base', do_lower_case=True)
	elif bert_mode == 'lap':
		tokenizer = RobertaTokenizer.from_pretrained('/laptop_pt/', do_lower_case=True)
	elif bert_mode == 'res':
		tokenizer = RobertaTokenizer.from_pretrained('/rest_pt/', do_lower_case=True)
...
def is_full_match(triplet, triplets):
	for t in triplets:
		if (t[0] == triplet[0] and t[1] == triplet[1]):
			return True
		# # if t[2] == triplet[2]:
		# if t[0] < triplet[1] or t[1] > triplet[0]:
		#	# 90% overlap
		#	if abs(min(t[1], triplet[1]) - max(t[0], triplet[0])) / (t[1] - t[0]) >= 0.9: 
		#		return True
	return False
...
def get_F1(data, preds, mode):
	...
	# print(gt_pos)
	# print(1/0)
	actual_correct = 2*correct_pos + total_samples - pred_pos - gt_pos
	return pred_pos, gt_pos, correct_pos, actual_correct, total_samples
...
class Encoder(nn.Module):
	def __init__(self, drop_out_rate):
		...
		if enc_type == 'roberta':
			if bert_mode == 'gen':
				self.BERT_model = RobertaModel.from_pretrained("roberta-base", output_attentions=True, output_hidden_states=False)
			elif bert_mode == 'lap':
				self.BERT_model = RobertaModel.from_pretrained("/laptop_pt/", output_attentions=True, output_hidden_states=False)
			elif bert_mode == 'res':
				self.BERT_model = RobertaModel.from_pretrained("/rest_pt/", output_attentions=True, output_hidden_states=False)
		elif enc_type == 'actual_bert':
			self.BERT_model = AutoModel.from_pretrained("bert-base-uncased", output_attentions=True, output_hidden_states=False)
		elif enc_type == 'distilbert':
			self.BERT_model = AutoModel.from_pretrained("distilbert-base-uncased", output_attentions=True, output_hidden_states=False)
		elif enc_type == 'electra':
			self.BERT_model = AutoModel.from_pretrained("google/electra-base-discriminator", output_attentions=True, output_hidden_states=False)
		elif enc_type=='mbert':
			self.BERT_model = AutoModel.from_pretrained("bert-base-multilingual-uncased", output_attentions=True, output_hidden_states=False)
		elif enc_type=='xlmr':
			self.BERT_model = AutoModel.from_pretrained("xlm-roberta-base", output_attentions=True, output_hidden_states=False)		
		elif enc_type=='mdistilbert':
			self.BERT_model = AutoModel.from_pretrained("distilbert-base-multilingual-cased", output_attentions=True, output_hidden_states=False)				
...
def train_model(model_id, train_samples, dev_samples, test_samples, test_gt_lines, best_model_file):
	...
	for epoch_idx in range(0, num_epoch):
		...
		if save_with_pp == 'y':
			pred_pos, gt_pos, correct_pos, actual_correct, total_samples = get_F1(dev_samples, dev_preds, 'test')
		else:
			pred_pos, gt_pos, correct_pos, actual_correct, total_samples = get_F1(dev_samples, dev_preds, 'dev')
		...
		dev_actual_acc = actual_correct/total_samples
		custom_print('Dev P:', round(dev_p, 3))
		custom_print('Dev R:', round(dev_r, 3))
		custom_print('Dev F1:', round(dev_acc, 3))
		custom_print('Dev ACC:', round(dev_actual_acc, 3))

		custom_print('\nTest Results\n')
		set_random_seeds(random_seed)
		test_preds = predict(test_samples, model, model_id)

		pred_pos, gt_pos, correct_pos, actual_correct, total_samples = get_F1(test_samples, test_preds, 'test')
		custom_print(pred_pos, '\t', gt_pos, '\t', correct_pos)
		test_p = float(correct_pos) / (pred_pos + 1e-8)
		test_r = float(correct_pos) / (gt_pos + 1e-8)
		test_acc = (2 * test_p * test_r) / (test_p + test_r + 1e-8)
		test_actual_acc = actual_correct/total_samples
		custom_print('Test P:', round(test_p, 3))
		custom_print('Test R:', round(test_r, 3))
		custom_print('Test F1:', round(test_acc, 3))
		custom_print('Test ACC:', round(test_actual_acc, 3))
	...
	custom_print('Corresponding ACC:', round(best_acc, 3))
	custom_print('\n\n')
	...
	custom_print('Corresponding ACC:', round(best_test_acc, 3))
	custom_print('\n\n')
...
if __name__ == "__main__":
	...
	parser.add_argument('--enc_type', type=str, default='roberta')
	parser.add_argument('--coarse_fine', type=str, default='coarse')
	parser.add_argument('--dom_nondom', type=str, default='dom')
	...
	tokenizer = getTokenizer(bert_mode, enc_type)
	batch_size = args.bs
	...
	rel_file = os.path.join(src_data_folder, f'{args.coarse_fine}_relation.txt')
	...
	if bert_mode == 'gen':
		f_train_sent = 'train.sent'
		f_train_pointer = f'{args.dom_nondom}_{args.coarse_fine}_train.pointer'
		f_train_nr_pointer = f'{args.dom_nondom}_{args.coarse_fine}_train.pointer'
		f_train_pos = 'trainb_pos.sent'
		f_train_dep = 'trainb_dep.sent'
		f_dev_sent = 'dev.sent'
		f_dev_pointer = f'{args.dom_nondom}_{args.coarse_fine}_dev.pointer'
		f_dev_pos = 'devb_pos.sent'
		f_dev_dep = 'devb_dep.sent'
		f_test_sent = 'test.sent'
		f_test_pointer = f'{args.dom_nondom}_{args.coarse_fine}_test.pointer'
		f_test_tuple = f'{args.coarse_fine}_testb_pt.tup'#'testb_pt.tup'
	...
	if job_mode == 'test':
		...
		pred_pos, gt_pos, correct_pos, actual_correct, total_samples = get_F1(test_data, test_preds, 'test')
		custom_print(pred_pos, '\t', gt_pos, '\t', correct_pos)
